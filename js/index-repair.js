...
if (localStorage.getItem('userId')) {
  ...
  showIndexReparatieSection();
}

// Login form handler
document.getElementById('index-loginForm').addEventListener('submit', async (e) => {
  e.preventDefault();
  
  const username = document.getElementById('index-loginUsername').value;
  const password = document.getElementById('index-loginPassword').value;
  ...
  try {
    ...
    if (data.success) {
      ...
      showIndexReparatieSection();
      showIndexMessage('Succesvol ingelogd!', 'success');
    } else {
      showIndexMessage(data.error, 'error');
    }
  } catch (error) {
    showIndexMessage('Er is een fout opgetreden', 'error');
  }
});

// Register form handler
document.getElementById('index-registerForm').addEventListener('submit', async (e) => {
  e.preventDefault();
  
  const username = document.getElementById('index-regUsername').value;
  const email = document.getElementById('index-regEmail').value;
  const password = document.getElementById('index-regPassword').value;
  ...
  try {
    ...
    if (data.success) {
      ...
      localStorage.setItem('username', data.username);
      showIndexReparatieSection();
      showIndexMessage('Account aangemaakt en ingelogd!', 'success');
    } else {
      showIndexMessage(data.error, 'error');
    }
  } catch (error) {
...
document.getElementById('index-telefoonMerk').addEventListener('change', function() {
  const merkSelect = this;
  const modelSelect = document.getElementById('index-telefoonModel');
  const andersTelefoonBeschrijving = document.getElementById('index-andersTelefoonBeschrijving');
  
  if (merkSelect.value === '') {
    modelSelect.disabled = true;
    modelSelect.innerHTML = '<option value="">Selecteer eerst een merk</option>';
    andersTelefoonBeschrijving.style.display = 'none';
    document.getElementById('index-andersTelefoonTekst').required = false;
    ...
    document.getElementById('index-andersTelefoonTekst').required = true;
  } else {
    modelSelect.disabled = false;
    modelSelect.innerHTML = '<option value="">Selecteer model</option>';
    
    const models = indexPhoneModels[merkSelect.value] || [];
...
// Problem radio button handler
document.querySelectorAll('input[name="index-probleem"]').forEach(radio => {
  radio.addEventListener('change', function() {
    const andersBeschrijving = document.getElementById('index-andersBeschrijving');
    if (this.value === 'Anders') {
      andersBeschrijving.style.display = 'block';
      document.getElementById('index-andersTekst').required = true;
    } else {
      andersBeschrijving.style.display = 'none';
      document.getElementById('index-andersTekst').required = false;
    }
  });
});

// Repair form handler
document.getElementById('index-repairForm').addEventListener('submit', async (e) => {
  e.preventDefault();
  
  if (!indexCurrentUser) {
    showIndexMessage('Je moet ingelogd zijn', 'error');
    return;
  }
  
  // Validate form data
  const klantNaam = document.getElementById('index-klantNaam').value;
  const telefoonMerk = document.getElementById('index-telefoonMerk').value;
  const telefoonModel = document.getElementById('index-telefoonModel').value;
  const probleem = document.querySelector('input[name="index-probleem"]:checked');
  const beschrijving = document.getElementById('index-beschrijving').value;
  const datum = document.getElementById('index-datum').value;
  const tijd = document.getElementById('index-tijd').value;
  
  if (!klantNaam || !telefoonMerk || !telefoonModel || !probleem || !datum || !tijd) {
    showIndexMessage('Vul alle verplichte velden in', 'error');
    return;
  }
  
  // Validate "Anders telefoon"
  if (telefoonModel === 'Anders telefoon') {
    const andersTelefoonTekst = document.getElementById('index-andersTelefoonTekst').value;
    if (!andersTelefoonTekst.trim()) {
      showIndexMessage('Vul het telefoon model in als je "Anders" selecteert', 'error');
      return;
    }
  }
  
  // Validate "Anders" problem
  if (probleem.value === 'Anders') {
    const andersTekst = document.getElementById('index-andersTekst').value;
    if (!andersTekst.trim()) {
      showIndexMessage('Beschrijf het probleem als je "Anders" selecteert', 'error');
      return;
    }
  }
  
  // Show confirmation
  indexToonBevestiging(klantNaam, telefoonModel.value, probleem.value, beschrijving, datum, tijd);
});

// Date change handler
document.getElementById('index-datum').addEventListener('change', async (e) => {
  const datum = e.target.value;
  
  if (!datum) {
    document.getElementById('index-tijd').innerHTML = '<option value="">Selecteer eerst een datum</option>';
    return;
  }
  ...
  try {
    ...
    const tijdSelect = document.getElementById('index-tijd');
    ...
    document.getElementById('index-tijd').innerHTML = '<option value="">Fout bij laden van tijden</option>';
  }
});

function showIndexReparatieSection() {
  document.getElementById('reparatie-login-section').style.display = 'none';
  document.getElementById('reparatie-form-section').style.display = 'block';
  document.getElementById('index-welcomeUser').textContent = indexCurrentUser.username;
}

function indexLogout() {
  indexCurrentUser = null;
  localStorage.removeItem('userId');
  localStorage.removeItem('username');
  document.getElementById('reparatie-login-section').style.display = 'grid';
  document.getElementById('reparatie-form-section').style.display = 'none';
  document.getElementById('index-bevestigingSection').style.display = 'none';
  
  // Reset forms
  document.getElementById('index-loginForm').reset();
  document.getElementById('index-registerForm').reset();
  document.getElementById('index-repairForm').reset();
}

function indexToonBevestiging(klantNaam, telefoonModel, probleem, beschrijving, datum, tijd) {
  document.getElementById('reparatie-form-section').style.display = 'none';
  document.getElementById('index-bevestigingSection').style.display = 'block';
  ...
  document.getElementById('index-confirmKlantNaam').textContent = klantNaam;
  document.getElementById('index-confirmTelefoonModel').textContent = telefoonModelTekst;
  document.getElementById('index-confirmProbleem').textContent = probleemTekst;
  document.getElementById('index-confirmBeschrijving').textContent = beschrijving || 'Geen extra opmerkingen';
  document.getElementById('index-confirmDatum').textContent = datumFormatted;
  document.getElementById('index-confirmTijd').textContent = tijd;
}

function indexWijzigGegevens() {
  document.getElementById('reparatie-form-section').style.display = 'block';
  document.getElementById('index-bevestigingSection').style.display = 'none';
}

async function indexBevestigReparatie() {
  if (!indexCurrentUser) {
    showIndexMessage('Je moet ingelogd zijn', 'error');
    return;
  }
  ...
  const klantNaam = document.getElementById('index-klantNaam').value;
  const telefoonModel = document.getElementById('index-telefoonModel').value;
  const probleem = document.querySelector('input[name="index-probleem"]:checked').value;
  const beschrijving = document.getElementById('index-beschrijving').value;
  const datum = document.getElementById('index-datum').value;
  const tijd = document.getElementById('index-tijd').value;
  ...
  try {
    ...
    if (response.ok) {
      showIndexMessage('Reparatie succesvol ingepland!', 'success');
      
      // Reset form and views
      document.getElementById('index-repairForm').reset();
      document.getElementById('index-tijd').innerHTML = '<option value="">Selecteer eerst een datum</option>';
      document.getElementById('index-andersTelefoonBeschrijving').style.display = 'none';
      document.getElementById('index-andersTelefoonTekst').required = false;
      document.getElementById('index-andersBeschrijving').style.display = 'none';
      document.getElementById('index-andersTekst').required = false;
      document.getElementById('reparatie-form-section').style.display = 'block';
      document.getElementById('index-bevestigingSection').style.display = 'none';
    } else {
...
function showIndexMessage(message, type = 'info') {
  const messageDiv = document.getElementById('index-message');
